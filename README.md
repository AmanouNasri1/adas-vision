...
| Throughput (end-to-end) | ~24 FPS |
| Processed frames | 900 |
| Total detections | 4,865 |
| Unique tracks | 191 |
| Risk track-frames (LOW / MED / HIGH) | 2,074 / 2,111 / 25 |
| High-risk (`brake_warning`) events | 25 |
...
### PyTorch vs ONNX (detection backend)

`yolo11n` exported to ONNX (`python apps\benchmark_onnx.py`), end-to-end `predict()`
(preprocess + inference + NMS), 80 runs on one frame:

| Backend | Device | FPS | ms/frame |
| --- | --- | --- | --- |
| PyTorch | CUDA (RTX 3050) | 71.4 | 14.0 |
| PyTorch | CPU | 22.0 | 45.5 |
| ONNX Runtime | CPU | 20.3 | 49.3 |

GPU is ~3.5× faster than CPU. On CPU, ONNX Runtime runs at roughly PyTorch-CPU speed
here — for a nano model the per-frame cost is dominated by pre/post-processing, so the
backend swap shows little delta; ONNX's real win is **portable, framework-independent
deployment** (no PyTorch needed at inference). Full table: [reports/onnx_benchmark.md](reports/onnx_benchmark.md).
